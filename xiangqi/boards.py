...
用法：
  python boards.py selftest           跑全部断言：规则细则、记谱、描述泄露、渲染、选项键、违规分类、两层物质、引擎可复现、局面集逐个复核
  python boards.py selftest --quick   同上，但随机对局交叉比对只跑少量
  python boards.py show GAME FEN      打印一个局面的特征与各档描述（调试用）
